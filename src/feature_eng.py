...
def add_last_campaign_started(df_users):
    ...
    campaigns = campaigns.rename(index=str, columns={'Campaign': 'last_campaign_started', 'Level':'last_level_started'})
    df_users = pd.merge(df_users, campaigns, how='left', on='last_level_started')
    df_users['last_campaign_started'] = df_users['last_campaign_started'].fillna('other')
    return df_users
...
def add_country_group(df_users):
    us = ['united-states']
    other_english = ['united-kingdom', 'australia', 'canada', 'new-zealand', 'ireland', 'singapore']
    df_users['country_us'] = df_users['Country'].isin(us)
    df_users['country_english_ex_us'] = df_users['Country'].isin(other_english)
    df_users['country_non_english'] = -(df_users['country_us'] | df_users['country_english_ex_us'])
    return df_users
...
if __name__=='__main__':
    ...
    df_users = add_time_and_days_per(df_users, df_levels, first_six_data, "first_six")
